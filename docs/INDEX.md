# Index System (Tier‑2 routing key index)

Status: **implemented (L0 + L1+ compaction + binary fuse filters)**.

This document describes the TieredStore routing‑key index used by the Bun+TS
server. The index is a secondary structure that accelerates key‑filtered reads
without changing the single‑log write path.
...
- Keep local state as a cache: if local SQLite + caches are deleted, the index
  can be rebuilt from R2 manifests + run objects.
...
1) **Consult local SQLite** (`index_runs`) to decide which index run files
...
source‑of‑truth remains the manifest + run objects in R2.
...
An L0 run covers **16 consecutive segments** (default span) and stores:
...
Binary layout (big‑endian, compatible with the Go format):
...
- filter_len (u32) — binary fuse filter bytes
  - data_len (u32)
- **Filter bytes** (`filter_len`) — binary fuse filter (fast negative lookup)
- **Records**
...
- `retired_runs[]`: retired runs pending deletion (with `retired_gen` / `retired_at_unix`)
...
When **`compaction_fanout`** runs exist at the same level with contiguous
segment ranges, they are merged into a higher‑level run:

- **L0 → L1**: 16 L0 runs → 1 L1 run (covers 256 segments by default)
- **L1 → L2**: 16 L1 runs → 1 L2 run (covers 4,096 segments by default)

Higher‑level runs use **postings lists** (relative segment ids) instead of
16‑bit masks. Each run includes a binary fuse filter.
...
- copied into `retired_runs` with `retired_gen` + `retired_at_unix`
...
- `streams/<hash>/manifest.json` (index state + run list)
- `streams/<hash>/index/<run-id>.idx` (run objects)
