...
R09 configured journal byte bounds: ops and audit queues now select the largest ordered prefix that fits both the 512-event cap and the originating runtime's request-body ceiling, including JSON brackets, commas, escaping, gap rows and operational cell attribution. A bounded per-row writer prevents serialization from allocating an unbounded intermediate row. Unselected events remain in the queue before the storage await; selected events retain their cancellation guard and original IDs. A single event that cannot fit an empty batch is explicitly counted as dropped, emits an error, and retains gap debt for the next durable append so later events cannot be permanently wedged behind it. The five new regressions cover exact encoded boundaries, a 64 KiB multi-pass drain with a demonstrably entered cancelled append and concurrent newer event, and oversized-event gap recovery for both journals.
