...
- **Request headers**: `Stream-Encryption-Key` (required), `Content-Type`, `Stream-TTL` (canonical decimal seconds, 0..=4294967295), `Stream-Expires-At` (RFC3339; mutually exclusive with TTL), `Stream-Closed: true` (close-on-create), `Stream-Forked-From`/`Stream-Fork-Offset`/`Stream-Fork-Sub-Offset` (forks), `Stream-Key-Version`. Rejected: `Stream-Ordering`/`Stream-Segments`/`Stream-Scaling` → 400 `unified_routing`.
...
Same handlers, `head_only=true`. **200** + `Content-Type`, `Stream-Next-Offset`, `Cache-Control: no-store`, `Stream-Closed: true` if closed, `Stream-TTL: <remaining secs>` when TTL+expiry set (may exceed 4294967295 for a window that predates the ceiling, legacy or inherited by a fork: the ceiling bounds admission, not reporting) (`src/http.rs:6772-6797`; fork: `6464-6482`; lineage: `8175-8184`, segment token). Never slides idle TTL. Metered: `meter_read(desc, 0, 0)` — one read op, zero bytes (`src/http.rs:6780-6782`). Errors as GET minus body/offset validation.
...
`product_create` (`src/product.rs:1372-1604`). Scope: `streams.create` (+ `watches.manage` if body carries watches). Body: typed JSON doc `{format:{kind:"json"|"bytes",contentType?}, expiry?:{idle}|{at}, watches?}`; `expiry.idle` is a positive duration of at most 4294967295 s (spec §13 service maximum), zero or larger 400 `invalid_config` (`deny_unknown_fields`; `parse_create_doc`, `src/product.rs:241-394`, all failures 400 `invalid_config`; >256 KiB config body also 400 `invalid_config`). Requires `Prisma-Encryption-Key` (400 `missing_key` / 400 `invalid_key`).
...
- **Errors**: 404 `not_found`; 503 `creating`; 400 `missing_key` / `invalid_routing_key` (>1024 B or not visible ASCII — the same rule as the `Prisma-Routing-Key` append header) / `invalid_producer`; 403 `wrong_key`; 409 `sealed` (conflicting claim); 409 `producer_sequence_reused` (final didn't close); 409 `sealing` (another live final-bearing claim; via `seal_error_response`, retryable=true); 503 `seal_incomplete` (retryable); 500 `internal` (also marked retryable=true by `seal_error_response`, `src/product.rs:2800-2814`); 413 `payload_too_large` (final record over ingest bucket capacity). Translated append errors from the final record pass through `translate_append_response`.
...
`product_scan` (`src/product/scan.rs`). Scope: `records.read` (deliberately NOT metadata.read). Requires key. Strict query: `cursor`, `maxBytes` (clamp 4096–8 MiB, default 4 MiB; bad value → 400 `invalid_max_bytes`).
...
- **Errors**: 429 `project_rate_limit` (+`retry-after`) while the project's read-byte bucket is in debt, checked at entry before the key; 400 `missing_key` / `invalid_cursor` (wrong kind, garbage, unknown segment); 410 `scan_expired` (cursor TTL 6 h, `SCAN_TTL_MS`); 403 `wrong_key`; 404 `not_found`; 503 `creating`; 409 `not_stream_owner`+`Streams-Replay-To` and other translated read errors via `translate_read_error`; 500 `internal`.
- **Metering**: `meter_read` once per page (payload bytes incl. peer-relayed pages) + project read-byte debit of the framed page bytes at the render site (`debit_read_bytes`).

### 2.7 `POST /v1/streams/{name}/records` — append (single)
`product_append` → `product_append_inner` (`src/product.rs:3128-3394`). Scope: `records.append`. Requires `Prisma-Encryption-Key`; optional `Prisma-Routing-Key` (≤1024 B of visible ASCII; longer, or present but not text such as obs-text bytes ≥0x80 → 400 `invalid_routing_key`, never the default key), producer triple. Body: JSON stream → one JSON value (wrapped `[value]`); bytes stream → opaque body (empty → 400 `empty_body`).
...
- **Metering**: raw-core `meter_read` (params.internal=false) + post-hoc project read debit (`check_read_quota` at entry, `debit_read_bytes` in `render_product_read` by framed body bytes). SSE variant is governed by subscription slot instead.
...
- **Mid-stream typed cutoffs** (owner move, incarnation change, incompatible topology, remote target/auth/redirect failures, and a live tail whose pinned engine retired under the same owner) end the body WITHOUT a terminal control; clients resume from their cursor; counted per reason under `/v1/debug/load` `sse_livefeed.cutoff_*` (`cutoff_engine_retired` added).
...
`product_consumer_pull` (`src/product/consumer_pull.rs`). Scope: `consumers.pull`. Body (optional) `{max?, waitMs?, visibilityMs?}` (`deny_unknown_fields`; 400 `invalid_body`); clamps: max 1..=`maxBatchRecords`, visibility 1 s–12 h (the lease window, `src/queue.rs`), wait ≤ 25 s (50 ms poll loop). **Read quota**: a batch is decrypted record payload, so the pull is admitted at entry against the project read-byte bucket (429 `project_rate_limit` + `Retry-After` while in debt, before any lease is taken) and the framed `{messages, backlog}` body is debited on success — the same bucket as records GET/long-poll and `:scan` (§17.2).
...
- **Metering**: `meter_pull` = queue_operations+1 + delivered payload bytes (`src/product/consumer_pull.rs`, `src/billing.rs:983-998`); the read-byte quota debit is the framed body (`debit_read_bytes`), separate from billing.
...
- **Errors**: 400 `invalid_watch_key` (not 16 hex); 400 `invalid_timeout_ms`; 404 `unknown_watch` (post-auth only); 503 `creating`; 500 `internal`; strict-query 400s. Ownership (post-auth, post-admission, before the journal is read): 409 `not_ring_owner` + `Streams-Replay-To` on a non-owner, 503 `shard_opening`/`shard_moving`/`shard_closing` + `Retry-After`, 500 `shard_open` (the shared `resolve_error_response` envelope).
...
- `POST /v1/internal/sweep-segment/{*name}` (`src/product.rs:5281-5378`) — body `{consumer, segId, fenceBelow, maxSteps}`; 200 `{"complete": bool, "steps": n}`; 400 `invalid_body` / `invalid_target`; 404 `not_found` (no descriptor for the name); 503 `temporarily_unavailable` (retryable — the registry could not be read; never a "gone" signal); 503 `segment_cleanup_failed`; ownership 409 passthrough.
- `GET /v1/internal/queue-cursor/{*name}` (`src/product.rs:5385-5449`) — headers `streams-internal-consumer`, `streams-internal-gen`; 200 `{"cursor": n, "tail": n}`; 400 `invalid_body`; 404 `not_found` (no descriptor for the name); 503 `temporarily_unavailable` (retryable — the registry could not be read; never a "gone" signal).
- `GET /v1/internal/segment-scan/{*name}` (`src/product.rs:5491-5604`) — headers `streams-internal-from`, `streams-internal-max-bytes` (clamped), `stream-encryption-key`; 200 `{"items": [{"off","rk","p"(b64)}], "last", "end", "completed"}`; 400 `invalid_body`; 403 `wrong_key`; 404 `not_found` (no descriptor for the name); 503 `temporarily_unavailable` (retryable — the registry could not be read; never a "gone" signal); 500.
- `POST /v1/internal/telemetry-append/{*name}` (`src/http.rs:1227`, `src/billing/system_append.rs::append_local`) — reserved `_`-streams only (403 `not_system_stream`); 400 `missing_key` (no `Stream-Encryption-Key` and no default key), `invalid_name` (reserved but not canonical), `invalid_key`; then the same TYPED local path the sender took: append, create on `not_found` under the SYSTEM project, append again. A refusal is the receiver's own decision, never a status guess: a non-owner receiver answers 409 `not_ring_owner` + `Streams-Replay-To` (the creation refusal, rendered by `creation_error_response`), an owner answers the raw append contract (200/201/204 + `Stream-Next-Offset` etc.). The sender (`billing::system_append`) relays ONCE and only on an owner-bearing refusal; any other refusal is reported as `system append {stream}: {failure}`.
...
- Reads: raw GET/HEAD + product records GET + scan → `meter_read` (op + payload bytes); SSE → `meter_read_chunk` at yield boundary. **Gap: `read_v3_lineage_inner` (split streams) has no `meter_read` call** — raw + product keyed reads of segmented streams are unmetered in billing (product still quota-debits served bytes via `debit_read_bytes` at the render sites).
