...
- [12. Incomplete multipart uploads on Tigris](#12-incomplete-multipart-uploads-on-tigris)
...
## 12. Incomplete multipart uploads on Tigris

**Finding (another agent's research, forwarded by the owner on 2026-09-25).**
Tigris does not support S3's `AbortIncompleteMultipartUpload` lifecycle
action (its Terraform provider says incomplete-multipart cleanup is
unsupported). It does support `ListMultipartUploads` and
`AbortMultipartUpload`; its pricing lists `ListMultipartUploads` as a billed
request, while DELETE/abort requests are free. There is anecdotal evidence
(a 2025 Tigris employee comment) that Tigris eventually cleans up and did
not bill uncommitted uploads, but no documented retention deadline or
guarantee.

**Why it matters here.** Nothing in this service aborts an incomplete upload,
so on Tigris the parts of every upload that never completes may be stored
(and possibly billed) indefinitely:
- SlateDB uploads SSTs and compaction outputs through `put_multipart` once
  they cross the multipart threshold (the 32 MiB rolls: `docs/CAPACITY-R27.md`
  records "multipart 0 -> 36").
- A process that dies mid-upload (item 38's bounded exit, an OOM kill,
  SIGKILL after a stop deadline, a Compute replacement) leaves the upload
  open.
- A cancelled upload is never aborted: the `object_store` crate's
  `MultipartUpload` does not abort on drop, and our timing wrapper
  (`src/store_timing.rs`, `TimedMpu`) only forwards an explicit `abort()`.
  `store_timing::tests::dropping_multipart_futures_records_cancelled` shows
  such drops happen.
- `docs/DST.md` already notes that a lost multipart response leaks an upload
  nothing can drive to completion.

**What to do.**
1. **Measure first.** Write a read-only audit (script or `bench` tool) that
   lists incomplete uploads under a bucket prefix (`ListMultipartUploads`,
   paged; `object_store` has no listing API, so use a signed S3 request with
   the credentials the campaign scripts already use) and reports count,
   bytes (`ListParts`) and age distribution. Run it against a campaign bucket
   that has seen crashes and kills. Keep only aggregates.
2. **Cleanup design** (owner decision on whether to rely on Tigris' own,
   undocumented cleanup instead):
   - A cell maintenance task (one owner per prefix, so not every instance
     lists): list incomplete uploads under the cell's prefixes and abort those
     initiated longer ago than a threshold far above any legitimate upload's
     duration (for example 24 h; a live SlateDB upload of a 32-64 MiB SST
     completes in seconds to minutes). Aborts are idempotent and free. An
     abort of an upload that is actually still live fails that writer's
     completion, so the age threshold is the safety argument; document it.
   - Optionally, abort on cancellation: make `TimedMpu` (or a wrapper beside
     it) abort a dropped, uncompleted upload on a detached task. This covers
     cancellations, not crashes, so the janitor is still needed. It changes
     store behaviour under cancellation: add it to the store fault model in
     `docs/DST.md`.
   - Test with `s3lite` (`src/bin/s3lite/emulator.rs` implements multipart
     initiate, complete and abort; it would need `ListMultipartUploads` and
     an initiation timestamp) and a DST fault that drops an upload mid-way.
3. Record the chosen behaviour in `docs/PROVIDER-CONTRACT.md` (the provider
   contract names multipart retries) and in RUNBOOK.
