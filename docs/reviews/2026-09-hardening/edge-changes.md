...
- **Condition:** The streamId is not an incarnation of {name} as the rollup recorded it in the request's own account and project: neither the live incarnation, nor listed in the month's name aggregate for {name} (the answer's `incarnations`), nor carrying {name} on its month row or any of its segment states. That covers another stream's id and an id that names nothing. (A first version of this fix checked only the month's aggregate and so also refused a prior incarnation of {name} in a month it did not contribute to, or whose storage the aggregate had not listed yet; a review found that regression and the rework restored those answers before the change was pushed.) It applies in every auth mode. Rollup keys carry the account and project, so the id could only ever select a row inside the request's own project.
...
- **After:** 404 with Content-Type application/json and Cache-Control: no-store. Body: {"error":{"code":"not_found","message":"streamId is not an incarnation of this stream","retryable":false}}. The id's month row is never answered (its month row and segment states are read only to compare their recorded name). The live id named explicitly and every prior incarnation of {name} are served exactly as before, including a zero row for a month it did not contribute to. A failed read of the name aggregate is 503 usage_unavailable (retryable), like every other rollup read failure on this route. Without streamId nothing changes.
- **Retry semantics:** 200 becomes a 404 with retryable:false, permanent for an id that is not an incarnation of {name} (an id's recorded name never changes). The TS SDK has no usage method, so only direct HTTP callers see it.
- **Who is affected:** Callers passing a streamId outside the name's incarnations for the month. That includes a credential reading another stream's usage through a name it may read, which is the bypass this closes, whether the other stream is outside its prefix grant or not. Callers naming any incarnation of the URL's name, in any month, are unaffected. Also changed: a correction that writes an incarnation's month row first now stamps the row with the incarnation's account and name and lists it in the name aggregate's `incarnations` for that month (before: an empty `streamName` on the stored row and no `incarnations` entry).
- **Pinning tests:**
