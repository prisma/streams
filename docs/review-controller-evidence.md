...
| Fleet populations | 4,096 provider entries, 128 KiB per body, 8 MiB total | Eight independent body fetches; 10-second population deadline | Incomplete reads publish no membership; retry reads the authoritative population. |
...
The configuration validator rejects fleet limits outside the same document population bound before topology-size arithmetic or startup. The operator view reports unavailable documents as unavailable. The outbox drainer returns actual body/decoding errors instead of treating them as empty outboxes.
