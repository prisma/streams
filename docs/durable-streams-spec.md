...
Malformed payloads return `400`. Unsupported content types or content encodings
return `415`. Accepted spans are appended as canonical JSON span records using
`traceId` as the routing key.
