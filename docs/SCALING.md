...
### 11.3 Deterministic simulation testing — step 1 shipped
...
passes because the race did not happen to fire). `src/dst.rs` now runs
our real `ShardEngine` against a seeded fault-injecting object store and
asserts four invariants, including a fencing-handoff scenario covering
the class that produced the pass-3 zombie-GC data loss. Remaining steps
(injectable clock, seeded fail points, multi-instance simulation) and the
honest limits of what simulation replaces: [DST.md](./DST.md).
