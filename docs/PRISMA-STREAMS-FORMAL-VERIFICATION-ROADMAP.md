...
|---|---|---|---|---|---|
| KANI-001 | pass-with-recorded-scope | 2 / 1 / 0 | Offset round trip over every segment ordinal below 2^30 and every `u64` `next` (`src/offsets/proofs.rs`), with the canonical-alphabet property in its own harness since 2026-09-26 (CBMC peaked near 15 GB with both in one; apart, under 5 GB), retargeted to slate's codec at the merge; the full-width finding is open (§0.4). The `String` wrapper stays with the unit tests (ASM-OFFSET-DOMAIN). | to be recorded at `d83af9a4` | — |
...
| KANI-017 | pass-with-recorded-scope | 2 / 3 / 0 | `decode_row`, and through it `decode_at` and `crypto::decode_frame` (`src/shard/record/proofs.rs`), over a 64-byte row of symbolic bytes (every version byte, offset and ciphertext length), a key of up to 26 symbolic bytes and the prefix `record_key` gives a symbolic stream hash: an admitted row's key is that prefix and one offset, its frame carries the key's offset, and the row is exactly one whole frame with its 16-byte tag, so a valid frame stored under another row's key is refused. The routing-key length is fixed per harness at 0 and 4 bytes. | recorded with its commit | — |
...
**Status:** pass-with-recorded-scope (implemented 2026-09-26; manifest entry KANI-017, 2 baselines and 3 negative controls: no offset comparison, a prefix check that ignores the record tag byte, and a frame-length check that allows trailing bytes). The routing-key length field is fixed per harness (0 and 4 bytes, the key's bytes symbolic): with a symbolic length the checker validates UTF-8 over every length and did not finish in 40 minutes. The row is 64 bytes, room for a version-4 frame with a 4-byte routing key and a ciphertext of 16 to 21 bytes and for longer legacy frames. No finding.
...
**Status:** pass-with-recorded-scope (implemented 2026-09-26; manifest entry KANI-028, 4 baselines and 3 negative controls: a tiling rule that accepts a one-point gap or overlap, a `contains` without the terminal-range convention, and a `route` without its sealed-cover fallback). The proof targets the coverage rule, not the whole `validate`: Kani could not finish `validate` over even two symbolic segments in 45 minutes, because the checker loses track of the (empty) lineage vectors held inside the map's vector, so the lineage loops and the length of the sort input stay symbolic, and a sort of symbolic length explores every sort strategy. Production changes, none changing behavior: the tiling rule is one function, `tiles_keyspace`, shared with `check_partition`, and `Registry::resolve_segment`'s choice of segment moved into `SegmentMap::route`; these made the proof possible. From the attempt to check `validate` whole, it also scans for duplicate identities instead of using hashed sets (whose seed comes from the OS random source, which Kani cannot model) and reports a typed `TopologyError` whose `Display` keeps every message, instead of formatting a `String` where it finds the fault. Those remove two obstacles for a harness over validation (KANI-031); the third, lineage vectors whose lengths the checker loses, remains. With a validated map's terminal segments tiling the key space, the `unreachable!` after `resolve_segment`'s choice cannot be reached. Duplicate identities and seal metadata stay with the unit tests, lineage and pending transitions with KANI-031. The open question this raised (`route` preferred the sealed cover with the latest `created_ms`, a wall clock `validate` does not order along lineage) led to a production finding, fixed on 2026-09-26 as edge change #60: keyed reads, producer and Stream-Seq dedupe, consumers and scans ordered lineage the same way, so clock skew between instances could serve a child before its parent, report an open stream closed, or write a retried append twice. Every site now uses `SegmentMap::lineage` (allocation order, which `validate` enforces), and `route` falls back to the highest-id sealed cover. No finding in the coverage rule itself.
