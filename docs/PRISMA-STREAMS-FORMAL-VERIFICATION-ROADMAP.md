...
| TLA-002 — seal claims, renewal and competing takeover reservations | counterexample | `SealProtocol.tla` (`MC_SealTakeover`). Claims, exact renewal, takeover reservation, fence and install, across crashes, engine replacement, crash failover between two instances, registry faults, cancellation, and a fence group that is staged before it is durable. Semantic properties compare every answer with the durable state: a live claim is never fenced, a queued final is decided before a takeover installs, a 2xx proves its outcome, and debt is released only when undeliverable. 11 baselines including 3 liveness shapes; 8 negative controls, 4 of them the pre-fix F1 engine; 14 witnesses; 2 known-defect checks. | **TLA-002-F1, production defect, fixed** in "A seal takeover's fence outlives the engine that recorded it". The fence lived only in the engine. After an ownership move or an engine replacement, a stalled, superseded final could close the segment under the takeover's claim, and the takeover's holder could then get 200 `sealed` for a record that was never written. Reproduced on the real code. **TLA-002-F2, open:** a gap in that fix. A `SealSuperseded` refusal was answered from the staged fence before the fence row was durable. If that group was lost, an older generation of the same operation could close the segment with no claim standing. |
...
**Status:** counterexample: one open defect, TLA-002-F2 (first spike; see [§0](#0-implementation-record)).
...
**Source owners:** [`src/history/gather.rs`](src/history/gather.rs); [`src/history/worker.rs`](src/history/worker.rs); [`src/shard/transaction/maintenance.rs`](src/shard/transaction/maintenance.rs); [`docs/HISTORY-V2.md`](docs/HISTORY-V2.md)
