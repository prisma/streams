...
impl ProjectId {
    ...
    #[expect(
        clippy::expect_used,
        reason = "ProjectId::stream_ref; callers pass names they already validated as canonical; a fallible lookup would add a branch no caller reaches"
    )]
...
#[expect(
    clippy::expect_used,
    reason = "system_project; the reserved system id is a constant that satisfies the project grammar; a fallible constructor would add a branch no caller reaches"
)]
