...
impl PostingsCache {
    ...
    #[expect(
        clippy::unwrap_used,
        reason = "PostingsCache::stats; a poisoned cache index may hold a partially installed slice or in-flight load; recovering it could serve a truncated postings slice or miscount resident bytes"
    )]
    ...
    #[expect(
        clippy::too_many_arguments,
        reason = "PostingsCache::maybe_prefetch; the prefetch takes the lookup's typed parts exactly as the read path resolved them; a request struct would exist for this single call site"
    )]
    #[expect(
        clippy::unwrap_used,
        reason = "PostingsCache::maybe_prefetch; a poisoned cache index may hold a partially installed slice or in-flight load; recovering it could serve a truncated postings slice or miscount resident bytes"
    )]
    fn maybe_prefetch(
        self: &Arc<Self>,
        part: &Arc<Db>,
    ...
    #[expect(
        clippy::unwrap_used,
        reason = "PostingsCache::sweep_idle; a poisoned cache index may hold a partially installed slice or in-flight load; recovering it could serve a truncated postings slice or miscount resident bytes"
    )]
