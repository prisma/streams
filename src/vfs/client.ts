...
export type VfsFetch = (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;

export type OpenVfsRepoOptions = {
  streamsUrl: string;
  stream?: string;
  tenantId?: string;
  repoId?: string;
...
export type VfsWorkspaceCommitOptions = Omit<VfsCommitRequest, "expectedHead"> & {
  expectedHead?: string | null;
};

export class VfsClientError extends Error {
  readonly status: number;
  readonly body: unknown;

  constructor(message: string, status: number, body: unknown = null) {
    super(message);
    this.name = "VfsClientError";
    this.status = status;
    this.body = body;
  }
}

export function vfsRepoStreamName(tenantId: string, repoId: string): string {
  return `vfs/${tenantId}/${repoId}/control`;
}
...
export class VfsRepoClient {
  ...
  constructor(options: OpenVfsRepoOptions) {
    ...
    if (!stream) {
      throw new VfsClientError("stream or tenantId/repoId is required", 0);
    }
  ...
  private async requestJson<T>(path: string, init: RequestInit = {}, params?: Record<string, string | number | boolean | null | undefined>): Promise<T> {
    ...
    if (!res.ok) throw new VfsClientError(errorMessage(body, `VFS request failed with ${res.status}`), res.status, body);
    return body as T;
  }
  ...
  private async requestBytes(path: string, init: RequestInit = {}, params?: Record<string, string | number | boolean | null | undefined>): Promise<Uint8Array> {
    ...
    if (!res.ok) {
      const text = await res.text();
      const body = decodeJsonMaybe(text);
      throw new VfsClientError(errorMessage(body, `VFS request failed with ${res.status}`), res.status, body);
    }
  ...
  async ensure(options: VfsEnsureOptions = {}): Promise<void> {
    ...
    if (!res.ok) {
      const text = await res.text();
      const body = decodeJsonMaybe(text);
      throw new VfsClientError(errorMessage(body, `VFS stream create failed with ${res.status}`), res.status, body);
    }
  ...
  async checkout(options: CheckoutOptions = {}): Promise<VfsWorkspace> {
    ...
    return new VfsWorkspace(this, res);
  }
  ...
  async maybeStat(path: string, opts: { commit?: string | null; workspaceId?: string | null } = {}): Promise<VfsNodeStat | null> {
    try {
      ...
      if (error instanceof VfsClientError && error.status === 404) return null;
      throw error;
    }
  }
...
export class VfsWorkspace {
  readonly repo: VfsRepoClient;
  readonly ref: string;
  readonly workspaceId: string;
  baseCommitId: string | null;
  rootTreeId: string | null;

  constructor(repo: VfsRepoClient, checkout: VfsCheckoutResponse) {
    this.repo = repo;
  ...
  async readFileBuffer(path: string): Promise<Uint8Array> {
    ...
    if (node.type !== "file" || !node.blobId) {
      throw new VfsClientError(`${path} is not a file`, 400);
    }
  ...
  async writeFile(path: string, content: string | Uint8Array, opts: { executable?: boolean; contentType?: string } = {}): Promise<void> {
    ...
    if (Result.isError(normalizedRes)) throw new VfsClientError(normalizedRes.error.message, 400);
...
async function renderPathDiff(
  ...
  if (before?.type === "dir" || after?.type === "dir") {
    if (!before) return `diff -- vfs ${path}\nnew directory\n`;
    if (!after) return `diff -- vfs ${path}\ndeleted directory\n`;
    return "";
  }
  let beforeText = "";
  let afterText = "";
  ...
  const out = [`diff -- vfs ${path}\n`, `--- a${path}\n`, `+++ b${path}\n`, "@@\n"];
  for (const line of beforeLines) out.push(`-${line}\n`);
  for (const line of afterLines) out.push(`+${line}\n`);
  return out.join("");
}

export function openVfsRepo(options: OpenVfsRepoOptions): VfsRepoClient {
  return new VfsRepoClient(options);
}
