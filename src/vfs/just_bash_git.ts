...
import type { VfsWorkspace } from "./client";
import { VfsClientError } from "./client";
import { normalizeRef } from "./model";
import type { VfsCommit } from "./types";

export type VfsGitCommandOptions = {
  getWorkspace: () => VfsWorkspace;
  setWorkspace?: (workspace: VfsWorkspace) => void;
  author: {
    id: string;
    name?: string;
  };
...
async function currentHead(workspace: VfsWorkspace): Promise<string | null> {
  const ref = await workspace.repo.getRef(workspace.ref);
  return ref.commitId;
}

export function createVfsGitCommands(options: VfsGitCommandOptions): CustomCommand[] {
  const git = defineCommand("git", async (args) => {
    ...
    try {
      ...
      if (error instanceof VfsClientError) return fail(error.message);
