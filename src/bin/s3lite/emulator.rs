//! The s3lite emulator core: stored objects, the request ledger and the S3
//! REST dispatch. The `s3lite` binary serves it on its listener; the
//! object-store provider contract suite
//! (`bootstrap::tests::provider_contract` in the service crate) compiles
//! this same file and serves it in-process, so the contract qualifies the
//! emulator the benches use rather than a copy.
//!
//! Every test build compiles this module as test code, so its `unwrap_used`
//! exceptions are expectations of the served binary only.
...
impl AppState {
    pub(crate) fn new(latency: Duration, discard_substr: Option<String>) -> Arc<Self> {
        Arc::new(AppState {
            latency,
            discard_substr,
...
/// The emulator's whole S3 surface as one router over `state`.
