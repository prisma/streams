...
pub(crate) struct Offset(pub Option<u64>);

impl Offset {
    pub(crate) const START: Offset = Offset(None);

    /// First record index covered by a read at this offset.
    pub(crate) fn scan_from(self) -> u64 {
        match self.0 {
            None => 0,
            Some(n) => n + 1,
        }
...
pub(crate) fn encode_ep(epoch: u32, o: Offset) -> String {
