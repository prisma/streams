...
#[expect(
    ...
    reason = "serve; the session takes the state, the surface, the descriptor, key, params, headers and lease parts as the handler authorized them; a request struct would restate the authorization"
)]
#[expect(
    clippy::too_many_lines,
    reason = "serve; the session is one subscribe, drive and stream sequence whose teardown depends on which step admitted it; splitting it would separate the steps from the teardown they order"
)]
#[expect(
    clippy::excessive_nesting,
    reason = "serve; the driver nests the lease, version and generation waits and the take verdicts inside the drive loop of the spawned task; flattening them would separate each wake from the verdict it produces"
)]
#[expect(
    clippy::let_underscore_must_use,
    reason = "serve; the changed waits are only registrations on watches the driver already owns; handled results would only restate the registration"
)]
#[expect(
    clippy::disallowed_methods,
    reason = "serve; the driver task is owned by the subscription it drives and ends with it; a supervised driver would tie a request-scoped task to the runtime supervisor"
)]
...
#[expect(
    clippy::unwrap_used,
    reason = "response_from_stream; the response builder only carries the fixed content-type and cache headers the session sets, so it cannot fail; a fallible build would add a branch no session reaches"
)]
