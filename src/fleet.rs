...
pub(crate) fn return_home_allowed(
    owned: usize,
    gain: usize,
    total: usize,
    active_n: usize,
...
#[expect(
    ...
    reason = "start; the fleet tick reads the authority once and derives the heartbeat, the desired count, the rebalance and the publications from that one read; splitting it would separate the decisions from the read that dates them"
)]
#[expect(
    clippy::excessive_nesting,
    reason = "start; the tick nests each decision inside the branch of the authority state that permits it; flattening them would separate the decisions from the state they were read under"
)]
#[expect(
    clippy::unwrap_used,
...
#[expect(
    ...
    reason = "start; the desired counts are ceilings of non-negative ratios bounded by the fleet maximum and the wedge seconds are clamped at zero; checked conversions would only restate those bounds"
)]
#[expect(
    clippy::cast_possible_truncation,
    reason = "start; the desired counts are ceilings of ratios bounded by the fleet maximum, far below u64; checked conversions would only restate that bound"
)]
#[expect(
