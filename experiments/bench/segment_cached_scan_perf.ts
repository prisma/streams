...
const FIXTURE_PATH = join(import.meta.dir, "../../test/fixtures/segments/remote-golden-stream-2/0000000000000123.bin");
...
function benchmark(rounds: number, fn: () => number): { rounds: number; totalMs: number; avgMs: number; decodedBlocks: number } {
  let totalMs = 0;
  let decodedBlocks = 0;
  ...
  return { rounds, totalMs, avgMs: totalMs / rounds, decodedBlocks };
}

const size = readFileSync(FIXTURE_PATH).byteLength;
const legacy = benchmark(50, () => legacyHotKeyScan(FIXTURE_PATH, size, HOT_KEY));
const warmup = benchmark(20, () => mappedHotKeyScan(FIXTURE_PATH, HOT_KEY));
const mapped = benchmark(200, () => mappedHotKeyScan(FIXTURE_PATH, HOT_KEY));
  ...
  JSON.stringify(
    {
      fixture: {
        path: FIXTURE_PATH,
        sizeBytes: size,
        sha256: fileSha256(FIXTURE_PATH),
      },
      legacy,
      warmup,
      mapped,
