...
- The release-soak judge refuses sub-24-hour evidence unless explicitly in
  ...
  files rotate off-path and every replacement is subject-pinned. CI proves the
  full harness with RS256/JWKS and durable tenant limits in short mode; no
  target-hardware release artifact has been produced.
...
5. Run the real mixed-version canary/rollback, preserving direct-instance
   capability verdicts plus semantic read/write/dark-restore evidence for each
   phase, and execute the checked 24-hour target-hardware release soak. Both
   judges and their hermetic CI proofs now exist; they do not substitute for a
   deployed old/new wave or target-hardware duration.
