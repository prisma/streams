...
| Availability and recovery | readiness distinct from liveness; stale-owner read guard; poison-shard quarantine; backup/PITR copy actor; restore and provider-failover drills with measured RPO/RTO | **Amber.** Readiness includes auth/revocation/audit/backup/fleet health; idle owners revalidate writer epoch within five seconds; repeated shard-open failures quarantine. Recovery points now pin every initialized shard and external ancestor, explicitly record absent lazy shards, expose only the selected manifest closure/compactions/WAL state, reuse content-addressed blobs, prune expired points and unreferenced blobs, and continuously hash referenced recovery content using a durable provider-independent cursor. CI restores adjacent points on either side of a durable append and proves reuse. A real independent-provider failover with measured RPO/RTO and continuous primary-side scrub evidence remain. |
...
- Recovery points hold expiring SlateDB checkpoints across topology rechecks,
  filter live and external DBs to exact manifest/compactions/WAL state, and
  treat lazy uninitialized shards as explicitly absent. Exact source ETags
...
3. Measure RPO/RTO in a real independent-provider failover drill and add
   ...
   recovery corpus already has checkpoint-pinned incremental points, bounded
   retention/GC, and rolling content verification.
