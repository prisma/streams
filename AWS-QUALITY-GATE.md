...
| Tenant isolation and authz | customer-scoped identity in every descriptor and request; locally verified scoped tokens; verb/prefix enforcement; no cross-tenant list/existence oracle; revocation; audit | **Amber.** Customer identity scopes registry, storage, routing, metrics, and requests. A real RS256/JWKS service drill proves identical-name isolation, per-tenant listing/non-disclosure, prefix/verb denial, live token revocation, and rollback-resistant revocation versions. Signed operator JWTs use the same issuer/audience/jti revocation validation, require an explicit default-false claim, and are limited to 15 minutes. Production admin mutations require a second signed operator with a distinct subject and token ID; missing, tenant, same-person, and overlong credentials fail before mutation. The immutable success record carries both identities, and denied attempts by a valid primary operator are durable without trusting an unverifiable second token. Pilot mode is explicitly exempt and cannot coexist with JWKS. An independent security review remains. |
...
| Horizontal scaling | automatic split/merge with quiesce proof; fleet aggregation; cell placement/isolation; hot-key behavior; no global coordination bottleneck at target scale | **Amber.** Online split and sibling merge use renewable shard-store intents, post-durability fences, verified clones, and one-CAS topology publication, with calibrated 60%/10% hot/cold triggers. A renewable epoch-fenced aggregator turns N bounded heartbeats plus router reports into one conditionally published `fleet.json`; servers and the pilot router consume that snapshot, and desired capacity includes the 32-shards-per-instance floor. Per-stream admission plus hierarchical owner scheduling bound hot-stream share. Managed cells validate strict generation-monotonic `cells.json`, use weighted rendezvous only for create, CAS a durable ≤4-cell customer affinity, and replay every wrong-cell operation before opening data. The operator-only move state machine requires fresh protocol-capable source/target fleets before preparing and cutover, durably fences and revalidates the source writer, copies exact raw encrypted stream ranges and checkpointed encrypted history without customer keys, verifies the target, and changes authority with one descriptor CAS; old/mixed fleets, crashes, and final-response retries fail closed or resolve idempotently. Retention-gated cleanup requires a provider-clock-aged descriptor, exact target recovery point, green backup/recovery/primary scrubs, and a fresh source fleet before deleting only obsolete source ranges/history while retaining permanent fences. Cell ops/data prefixes and global-registry credentials are distinct; production JWT is mandatory. A create-before-publish cell index makes backup/scrub enumeration O(cell) and crash-safe. CI exercises a live two-cell encrypted-history move, stale-source denial, target continuation/restart, provider-proof cleanup, permanent-fence preservation, and recovery. A dedicated IAM checker proves 21 own-namespace operations plus 42 explicit cross-registry/cross-cell denials and rejects a deliberate overgrant under the scoped emulator. A real-provider run and target-scale evidence remain. |
| Availability and recovery | readiness distinct from liveness; stale-owner read guard; poison-shard quarantine; backup recovery-point actor; restore and provider-failover drills with measured RPO/RTO | **Amber.** Readiness includes auth/revocation/audit/absorber/backup/fleet health; idle owners revalidate writer epoch within five seconds; repeated shard-open failures quarantine. Exact history-debt markers are atomic with record and absorbed-frontier commits; replacement owners reconstruct the bounded work set before admission and conservatively migrate unmarked tails, so idle debt survives process movement without a new append. Recovery points pin every initialized shard, active history DB, and external ancestor; explicitly record absent lazy DBs; expose only selected manifest closure/compactions/WAL state; reuse content-addressed blobs; prune expired points/unreferenced blobs; and continuously hash referenced recovery content using a durable provider-independent cursor. Managed points eagerly protect the exact O(cell) registry closure—directory, authoritative markers/descriptors, and affinities—before history pinning. Offline merge accepts identical duplicate objects and fails on conflicts, letting independently restored cell data rejoin one global registry without a global scan. Pre-manifest acknowledged WALs are ETag-fenced and copied immediately when each DB cut is observed, so source GC cannot overtake the later inventory walk. A clock-independent renewal/observation lease epoch-orders mutable backup publications; followers require observed liveness plus post-observation relative-age health. Format 3 checksum-rehomes unchanged blobs into epoch-specific paths, making a delayed old-epoch delete physically unable to damage its successor. A keyed writer wrapper create-only publishes each transformed history SST digest before object visibility, then logical verification precedes the absorbed-frontier commit; every crash point leaves either safe orphans or a keyless-scrubbable referenced object. A second durable cursor logically decodes live shard manifests/SSTs/WALs and checks customer-key history ciphertext against those writer-verified digests. Primary failure invalidates snapshot health until a full repaired sweep publishes a new point. History enumeration fails closed above the 100,000-DB cell bound. CI restores adjacent points, deletes an eagerly protected WAL from primary before inventory, proves coordinator takeover, corrupts live shard plus encrypted-history SSTs, and destroys/restores a managed cell plus its registry authority before the first decrypted read. The generic drill checks both providers' required S3 semantics, cuts the primary, measures the exact recovered-record RPO and first-decrypted-read RTO, and proves post-activation writes. Its latest two-process emulator run measured 8.751 s/519 ms; a real independent-provider run remains. |
| Operability and SLOs | RED metrics by tenant/cell/shard; bounded-cardinality telemetry; actionable alerts; audit trail; capacity model; on-call runbooks exercised by game days | **Amber.** Exact bounded per-tenant/stream RED and usage intervals are retry-stably appended to an encrypted service stream; exporter health and cardinality loss alarm without recursively metering that exact principal. Tenant control and state-changing operator audit is synchronously immutable on primary plus an independently credentialed mirror. Every read-only privileged request carries its verified principal/token/resource/result into an unsampled queue that flushes after 60 seconds or 256 events, so ordinary 15-second scrapes share one object rather than each creating one; sampled tenant audit retains an independent one-second writer/readiness bit. Bounded-queue rejection fails the privileged request. Every response gets a service-generated 128-bit request ID (caller values are stripped), version-1 audit records carry the same join key, and SDK errors expose it. Batches retry one stable object, conditionally persisted cursors reconcile all three prefixes before readiness, and provider-clock retention deletes primary only after byte verification. An operator-authenticated OpenMetrics endpoint exports fixed-label HTTP RED, active-tail freshness, exact absorber backlog, conservative protected-point age, fencing, component/backup health, WAL/memory, process CPU, active fleet size, a fixed 35-cell object-store attempt matrix, L0 compaction debt, unflushed-WAL recovery debt, and bounded per-open-shard signals. Twenty checked alerts carry severity, blast radius, and runbook; CI validates their schema and drills the SLO signals, stale topology, privileged audit, audit-provider loss, and retry-stable billing recovery without tenant labels. Real independent accounts/consumer reconciliation and a real notification-path game day remain. |
...
- Every HTTP response enters a fixed operation/status-class counter and fixed-
  ...
  drops, overload shedding, resident memory, monotonic process CPU, WAL PUT
  p50/p99, outbound store concurrency, a fixed operation/path-class matrix of
  finished store-call attempts, and the bounded open-shard set's durable-wait p99 and
  append counters plus L0 and unflushed-WAL debt. It never emits customer or
  stream names. Twenty checked JSON/YAML-
...
- Audit control records are acknowledged only after identical immutable writes
  ...
  cursors reconcile control, sampled, and operator-batch prefixes on every boot before readiness;
...
- Managed-cell serving now has a strict bounded directory, deterministic
  weighted placement, durable one-to-four-cell tenant affinity, immutable
  descriptor pinning, and wrong-cell replay before data access. A separately
  credentialed global registry is mandatory; each cell's ops/data prefix is
  exact and production JWKS authentication is required. Immutable cell indices
  are written before descriptor publication and revalidated during bounded
  backup/scrub discovery, so losing races leave only safe orphans and cells do
  not scan one another. Negative descriptors are never cached across cells.
  The offline migration tool audits, CAS-backfills, and re-audits a quiesced
  one-cell registry before a second cell may be admitted. The two-cell CI drill
  proves cutover, replay, no wrong-cell shard opens, index publication, affinity,
  same-generation poison handling, and last-known-good reads. It then destroys
  both serving cells, merges an exact cell registry closure into a clean global
  registry, restores the cell-local point independently, and proves the first
  decrypted read from recovery storage alone. The same drill moves a live
  absorbed stream between physical cells without giving the mover its key,
  rejects a stale-source append, continues offsets at target, retries the lost
  completion response, proves a target recovery point before provider-clock
  retention cleanup, preserves the permanent source fence while reclaiming the
  exact source ranges/history, and restores the moved target point.
- `streams-cell-iam-check` uses independently identified registry/cell
  ...
  Real provider policy evidence remains mandatory.
- The release-soak judge refuses sub-24-hour evidence unless explicitly in
  short test mode. It drives a victim plus a distinct-tenant oversized-write
  attack, reconciles every victim stream's durable offset, scrapes every named
  instance, rejects duplicate or incomplete active-fleet coverage, establishes
  a quiescent idle CPU/store baseline, and checks attacker 429 isolation,
  separate ACK p50/p99/p99.9 budgets, errors/throughput, loaded provider
  attempts per 1,000 exact records, readiness, RSS growth, absorber drain,
  L0/WAL debt, fences, and RPO before writing a secret-free JSON verdict. The
  three cost budgets are mandatory for production evidence, and the fixed
  35-cell store delta remains in the artifact. Mode-0600 JWT
  victim, attacker, and operator files rotate off-path and every replacement
  is pinned to one of three distinct JWT subjects. Stream keys are file-only;
  the harness rejects legacy raw-secret environment variables. CI proves the
  full harness with RS256/JWKS, signed default-false operator authorization,
  deliberate raw-secret rejection, and durable tenant limits in short mode;
  no target-hardware release artifact has been produced.
- Every fleet heartbeat now preserves an immutable release ID and explicit
  live/history/recovery reader/writer, ring, backup-coordination, and cell-move
  protocols. An operator-only endpoint exposes the local declaration plus the
  bounded fresh aggregate without tenant or shard identities. The
  mixed-version judge requires direct views from the exact cell membership,
  exact release composition, uniform writers, pairwise read compatibility,
  and protocol agreement before creating a mode-0600 secret-free verdict.
  Every server also fails fleet readiness on a known pairwise conflict, or on
  an unknown member after writers move beyond the 2/1/2 adoption baseline.
  Legacy heartbeats deserialize as unknown and cannot pass the external gate.
  CI exercises both layers through aggregate-leader loss and corrupt aggregate
  recovery. A real deployed old/new semantic rollout remains release evidence.
- The stateful mixed-version semantic canary binds every phase to its direct-
  instance capability verdict and active history/recovery writers. Read-first,
  canary-flip, rollback, and finalization each exact-retry a deterministic
  producer sequence and require every configured ingress to return the exact
  accumulated corpus. Its immutable evidence/state chain recovers the crash
  window between evidence creation and state replacement without duplicating
  a record; JWT subjects remain pinned across token rotation. CI forces each
...
4. Prove the implemented multi-cell placement/control plane against real
   provider IAM policies by preserving a passing `streams-cell-iam-check`
   artifact for every bucket role (including denied cross-cell writes), run it
   at the documented cell/tenant counts, exercise the bounded registry-union recovery
   against independent providers, and exercise the now-enforced retention-
   qualified cleanup under real provider policies and production scale before
   any high-volume production rebalancing wave.
5. Run the real mixed-version canary/rollback, preserving direct-instance
   capability verdicts plus the chained semantic, forced-absorption,
   at-rest/scrub, and dark-restore evidence for each phase, and execute the
   checked 24-hour target-hardware release soak with approved idle and loaded
   provider-cost budgets. Both judges and their hermetic
   CI proofs now exist; they do not substitute for a deployed old/new wave or
   target-hardware duration.
