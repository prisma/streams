...
| Performance and cost | repeatable target-hardware tests for p50/p99/p99.9, recovery, compaction, absorption lag, idle cost, noisy-neighbor isolation, and 24 h+ soak with regression budgets | **Red.** The repeatable judge records exact durable victim offsets, ACK p50/p99/p99.9, throughput/errors, readiness, RSS growth, absorber drain, L0/WAL debt, fences, RPO, and a concurrent distinct-tenant oversized-write attack whose every request must be a scoped 429. Victim, attacker, and operator JWT files rotate with three pinned distinct subjects; keys and tokens are file-only and absent from evidence. No qualifying 24-hour target-hardware artifact or idle-cost evidence exists yet. |
