...
| Operability and SLOs | RED metrics by tenant/cell/shard; bounded-cardinality telemetry; actionable alerts; audit trail; capacity model; on-call runbooks exercised by game days | **Amber.** Tenant-scoped bounded billing metrics and immutable durable audit records exist, with audit health in readiness. An operator-authenticated OpenMetrics endpoint exports fixed-label HTTP RED, component/backup health, WAL/memory, and bounded per-open-shard signals. An initial ten checked alert rules carry severity, blast radius, and runbook; CI validates their schema and proves a stale-topology unready signal/recovery without leaking tenant labels. Tail-freshness, absorber-lag, recovery-point-age, and fence-rate signals; independent audit/billing export; audit retention; and a real notification-path game day remain. |
...
- Every HTTP response enters a fixed operation/status-class counter and fixed-
  ...
  counters. It never emits customer or stream names. Ten checked JSON/YAML-
  compatible Prometheus alerts cover telemetry loss, component/backup failure,
  audit loss, fast/slow append error-budget burn, durable/WAL latency, memory,
  and shedding, each with severity, cell blast radius, and runbook. A black-box
  game day requires operator auth, real append/shard samples, last-known-good
  service with a replayed stale topology, an exact topology-unready metric plus
  readiness 503, and recovery after restoring v2.
...
6. Add tail-freshness, absorber-lag, protected-recovery-point-age, and fence-
   rate signals plus independently durable audit/billing export with bounded
   retention. Then exercise the deployed collector, alert evaluator,
   notification route, inhibition, and ownership labels in a real on-call game
   day. The bounded scrape, initial checked rule catalog, and hermetic stale-
