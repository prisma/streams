...
| scaling model | all 4 servers always awake, rendezvous over everything (wrong model) | fleet publishes its own `desired.json` from heartbeats; router routes to exactly that many; the rest **scale to zero** (verified: sleeping servers, single-server wake, full staircase 1→4 twice, descent, crash detection ≤10 s) |
