...
Upstream suite: `@durable-streams/server-conformance-tests` (239 tests; the
copy vendored in this repo's TS implementation). Run 2026-07-08 against
`streams-slate` with s3lite (5 ms injected latency).

## How to run

```bash
# emulator + server (fresh bucket per run — the suite assumes a clean namespace)
./slate/target/release/s3lite --listen 127.0.0.1:9500 --latency-ms 5 &
KEY=$(./slate/target/release/streams-keys generate)
./slate/target/release/streams-slate --listen 127.0.0.1:8090 \
  --s3-endpoint http://127.0.0.1:9500 --bucket conf-$RANDOM \
  --conformance-default-key "$KEY" &

# suite (repo-root vitest wiring; the package's npx CLI also works where
# vitest is on PATH)
CONFORMANCE_TEST_URL=http://127.0.0.1:8090 \
  ./node_modules/.bin/vitest run -c vitest.conformance.config.ts
```
